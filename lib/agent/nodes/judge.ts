...
export async function judgeNode(state: InvestmentAnalysisState): Promise<Partial<InvestmentAnalysisState>> {
  ...
  const fallback = () => ({
    ...
    reasoning: `Judge evaluated bull case (${state.bullConfidence}% confidence) vs bear case (${state.bearConfidence}% confidence). Composite score ${state.compositeScore}/100 with ${state.riskTolerance} risk tolerance yields ${fallbackVerdict}. Evidence from ${state.citations.length} sources.`,
